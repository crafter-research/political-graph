...
function Scene({
  ...
  useFrame((_, delta: number) => {
    ...
    if (modeRef.current === "manual") {
      ...
      else { const base = 0.38 + 0.07 * Math.sin(t * 0.7); targetIn = clamp01(base + 0.05 * Math.sin(t * 2.1) * Math.sin(t * 0.37 + 1.2)); targetOut = clamp01(0.48 + 0.12 * Math.sin(t * 1.05 + 0.6)) }
    }
...
void main() {
    ...
    float originalCenters[7]; originalCenters[0]=0.0; originalCenters[1]=0.5*PI; originalCenters[2]=1.0*PI; originalCenters[3]=1.5*PI; originalCenters[4]=2.0*PI; originalCenters[5]=2.5*PI; originalCenters[6]=3.0*PI;
    float centers[7];
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if (drawOval(vec2(distTheta, radius), vec2(0.0, 0.0), a, b, rev, 0.6, ovalColor)) {
